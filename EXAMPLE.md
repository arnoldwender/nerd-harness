...
and now nothing watches it. A regression shipped wearing a checkmark. You lied to the
report; you can't lie to the machine.

## With the Codex — the discipline fires

Two rules catch this before it lands:

- **Finish the Hack 3 — "Refuse the cheap rescue."** *No silenced test, no `@ts-ignore`, no
  `// for now` that fakes green.*
  **Falsifier:** *a test passes only because it was skipped, weakened, or had its assertion
  deleted.*
- **Read the Source 4 — "A guess in a lab coat is still a guess."** *Verify before you act;
